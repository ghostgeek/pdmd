...
public class ExcelUtils {
    ...
    // 构造方法，传入要导出的数据
    public ExcelUtils(String title, String[] rowName,
    ...
    public void export(OutputStream out) throws Exception {
        try {
            ...
            for (int i = 0; i < dataList.size(); i++) {
                ...
                XSSFRow row = sheet.createRow(i + 2);// 创建所需的行数（从第三行开始写数据）
            ...
            for (int colNum = 0; colNum < dataList.get(1).length; colNum++) {
                ...
                for (int rowNum = 0; rowNum < sheet.getLastRowNum(); rowNum++) {
                    ...
                    /*
                     * if (currentRow.getCell(colNum) != null) { XSSFCell
                     * currentCell = currentRow.getCell(colNum); if
                     * (currentCell.getCellType() == XSSFCell.CELL_TYPE_STRING)
                     * { int length =
